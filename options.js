...
async function checkGoogleAuthStatus() {
    try {
        ...
        // Show loading state
        showGoogleStatus('warning', '인증 상태 확인 중...');
        
        const response = await chrome.runtime.sendMessage({
            action: 'checkGoogleTokenStatus'
        });
        
        console.log('Google auth status response:', response);
        
        if (response.isValid) {
            showGoogleStatus('success', '인증됨 - Google Calendar 연동이 활성화되었습니다');
            authenticateGoogleBtn.classList.add('hidden');
            revokeGoogleBtn.classList.remove('hidden');
        } else {
            showGoogleStatus('error', '미인증 - Google 계정 인증이 필요합니다');
        ...
        showGoogleStatus('error', '인증 상태 확인 중 오류가 발생했습니다');
        authenticateGoogleBtn.classList.remove('hidden');
        revokeGoogleBtn.classList.add('hidden');
    }
}
